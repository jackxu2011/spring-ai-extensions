...
public class MessageDeserializer extends JsonDeserializer<Message> {
    ...
    MESSAGE_FACTORIES.put(
        ...
        ((textContent, metadata, toolCalls, toolResponses) ->
            AssistantMessage.builder()
                ...
                .toolCalls(Optional.ofNullable(toolCalls).orElseGet(List::of))
                .build()));
    MESSAGE_FACTORIES.put(
        "SYSTEM",
        ((textContent, metadata, toolCalls, toolResponses) ->
    ...
    MESSAGE_FACTORIES.put(
        ...
        ((textContent, metadata, toolCalls, toolResponses) ->
            ToolResponseMessage.builder()
                ...
                .metadata(Optional.ofNullable(metadata).orElseGet(Map::of))
                .build()));
  }
        ...
        .orElseGet(
            () ->
                Optional.ofNullable(node.get("type"))
                    ...
                    .orElseGet(
                        () ->
                            Optional.ofNullable(node.get("role"))
  ...
  private String extractContent(JsonNode node) {
    return Optional.ofNullable(node.get("content"))
        ...
        .orElseGet(
            () ->
                Optional.ofNullable(node.get("textContent"))
                    .map(JsonNode::asText)
  ...
  private List<AssistantMessage.ToolCall> extractToolCalls(JsonNode node) {
    return Optional.ofNullable(node.get("toolCalls"))
  ...
  private List<ToolResponseMessage.ToolResponse> extractToolResponses(JsonNode node) {
    return Optional.ofNullable(node.get("responses"))
        .filter(JsonNode::isArray)
        .map(
            toolResponsesNode -> {
              try {
